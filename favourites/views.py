...
def add_product_to_favourites(request, item_id):
    ...
    if product in favourites.products.all():
        messages.info(request, 'The product item is already in your favourites!')
    else:
...
def remove_product_from_favourites(request, item_id):
    ...
    if product in favourites.products.all():
        ...
        messages.info(request, 'Product item removed from your favourites list')
    else:
        messages.error(request, 'That product item is not in your favourites list!')
    return redirect(reverse('product_detail', args=[item_id]))
